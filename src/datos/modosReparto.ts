import type { ModoReparto, ModoRepartoId } from '../tipos';
import { CARTAS_POR_RONDA_DEFECTO } from '../constantes';

export const MODOS_REPARTO: ModoReparto[] = [
  {
    id: 'clasico',
    nombre: 'Clásico',
    icono: '🟢',
...
export function obtenerCartasPorRondaDelModo(
  idModo: ModoRepartoId,
  cartasPersonalizadas: number[]
): number[] {
  if (idModo === 'personalizado') {
    return cartasPersonalizadas.length > 0
      ? [...cartasPersonalizadas]
      : [...CARTAS_POR_RONDA_DEFECTO];
  }
  const modo = MODOS_REPARTO.find((m) => m.id === idModo);
  return modo?.cartas ? [...modo.cartas] : [...CARTAS_POR_RONDA_DEFECTO];
}
